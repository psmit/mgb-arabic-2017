...
#                    boundary_states.add(a.nextstate)
            if not sym.startswith('+'):# and s not in boundary_states:
                boundary_states.add(s)
    ...
    def dfs(state, cur_seq):
        ...
        for a in f.arcs(state):
            begin_phone = not st.find(a.ilabel).startswith("+")
            if (len(cur_seq) == 0 and begin_phone) or (len(cur_seq) > 0 and not begin_phone):
