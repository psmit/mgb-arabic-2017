...
theanolm_lmscale=10.0
theanolm_beam=650
theanolm_maxtokens=80
...
old_nj=`cat $indir/num_jobs` || exit 1;
...
$cmd JOB=1:$nj $outdir/log/theanolm.JOB.log \
      gunzip -c $indir/lat.*.gz \| \
      lattice-copy --include=${sdata}/JOB/utt2spk ark:- ark:- \| \
      lattice-scale --lm-scale=-1 --acoustic-scale=-1 ark:- ark:- \| \
      lattice-compose ark:- "fstproject --project_output=true $oldlm |" ark:- \| \
      lattice-determinize ark:- ark:- \| \
      lattice-scale --lm-scale=-1 --acoustic-scale=-1 ark:- ark,t:- \| \
      theanolm rescore --log-file $outdir/log/theano_rescore.JOB.log --log-level debug $newlm - $oldlang/words.txt - --lm-scale $theanolm_lmscale --beam $theanolm_beam --max-tokens-per-node $theanolm_maxtokens --recombination-order $theanolm_recombination \| \
