...
run=$srun
prev=NONE
if $run || [ ! -f data/recog_langs${lsuf}/${model}_egymix1/G.fst ]; then
    echo "Run: $run , or data/recog_langs${lsuf}/$model/G.fst doesn't exist"
    job process_lm 30 1 NONE common/process_lm_egy.sh $model gr
    run=true
    prev=LAST
fi

if $run || [ ! -f $am/graph_${model}_egymix1/HCLG.fst ];  then
    echo "Run: $run , or $am/graph_$model/HCLG.fst"
    job mkgraph 15 4 $prev -- utils/mkgraph.sh --self-loop-scale 1.0 data/recog_langs${lsuf}/${model}_egymix1 $am $am/graph_${model}_egymix1
    run=true
    prev=LAST
fi
...
if $run || { [ ! -f $am/decode${extra}_${model}_egymix1/scoring_mrwer/best_mrwer ] && [ ! -f $am/decode${extra}_${model}_egymix1/scoring_kaldi/best_wer ]; }; then
   
   job recognize_$model 4 4 $prev -- common/chain_recog_iter_graph_egy.sh --dataset $dataset --iter $iter $am data/recog_langs${lsuf}/${model}
   run=true
...
if [[ $model == "morfessor_f2_a1_types_pre" ]]; then
mem=40
tlbeam=500
fi

if [ ! -f $am/decode${extra}_${model}_egymix1_rnn_egy/num_jobs ]; then
  if [ -f data/lm/$model/egy/nnlm.h5 ]; then
    mkdir -p data/recog_langs${lsuf}/${model}_egy
    if [ ! -e data/recog_langs${lsuf}/${model}_egy/nnlm.h5 ]; then
      ln -rs data/lm/$model/egy/nnlm.h5 data/recog_langs${lsuf}/${model}_egy/nnlm.h5 
    fi
    job rescore_rnn 4 24 $prev -- common/lmrescore_theanolm_b.sh --beam 8 --scoring-opts "--min-lmwt 4" --lmscale 8.0 --theanolm-beam $tlbeam --theanolm-recombination $tlrecomb --theanolm-maxtokens $tlmaxtok --cmd "slurm.pl --mem ${mem}G"  data/recog_langs${lsuf}/${model}_egymix2 data/recog_langs${lsuf}/${model}_egy data/$dataset ${am}/decode${extra}_${model}_egymix1_egymix2 ${am}/decode${extra}_${model}_egymix1_rnn_egy
  fi
fi
#continue
#prev=LAST
...
if [ -f $am/decode${extra}_${model}_egymix1_rnn_bg/scoring_mrwer/best_mrwer ] && [ ! -f ${am}/decode${extra}_${model}_egymix1_rnn_bg_interp/best/scoring_mrwer/best_mrwer ] ; then
...
if [ -f $am/decode${extra}_${model}_egymix1_rnn_egy/scoring_mrwer/best_mrwer ] && [ ! -f ${am}/decode${extra}_${model}_egymix1_rnn_egy_interp/best/scoring_mrwer/best_mrwer ] ; then
...
if [ -f ${am}/decode${extra}_${model}_egymix1_rnn_bg_interp/best/scoring_mrwer/best_mrwer ] && [ -f ${am}/decode${extra}_${model}_egymix1_rnn_egy_interp/best/scoring_mrwer/best_mrwer ] && [ ! -f ${am}/decode${extra}_${model}_egymix1_rnn_both_interp/best/scoring_mrwer/best_mrwer ]; then
   job interp_both 3 4 NONE -- common/interpolate_2lattices_simple.sh --start 3 --scoring-opts "--min-lmwt 4" data/$dataset data/recog_langs${lsuf}/${model}_egymix2 ${am}/decode${extra}_${model}_egymix1_rnn_bg_interp/best ${am}/decode${extra}_${model}_egymix1_rnn_egy_interp/best ${am}/decode${extra}_${model}_egymix1_rnn_both_interp  
fi
