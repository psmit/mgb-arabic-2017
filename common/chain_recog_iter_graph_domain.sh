...
if [ ! -f ${am}/decode${extra}_${graph}_domainmix2/wer_15_0.0 ]; then 
echo "rescore result not there yet"
echo "${rl}_domainmix2/G.carpa"
if [ -f ${rl}_domainmix2/G.carpa ]; then
echo "There is a carpa!!"
  steps/lmrescore_const_arpa.sh --cmd "run.pl --max-jobs-run 19" --skip-scoring $skip_scoring --scoring-opts "--min-lmwt 4 --max-lmwt 18" ${rl}_domainmix1 ${rl}_domainmix2 ${am}/feats/${dataset} ${am}/decode${extra}_${graph} ${am}/decode${extra}_${graph}_domainmix2
fi
fi
...
echo "There is a carpa!!"
  steps/lmrescore_const_arpa.sh --cmd "run.pl --max-jobs-run 19" --skip-scoring $skip_scoring --scoring-opts "--min-lmwt 4 --max-lmwt 18" ${rl}_domainmix1 ${rl}_domain ${am}/feats/${dataset} ${am}/decode${extra}_${graph} ${am}/decode${extra}_${graph}_domain
fi
fi
